...
function RuneButtonC_Update(self, rune)
	...
	if class ~= "DEATHKNIGHT" then
		self:Hide()
		return -- Stop here if not a DK
	end

	-- Show the rune button (just in case)
	self:Show()

	-- Set rune texture and styling
...
function RuneFrameC_OnUpdate(self)
	...
	-- Try to assign references if not already assigned
	if not runePowerStatusBar then
		runePowerStatusBar = _G["RunePowerStatusBar"]
	end
	if not runePowerStatusBarText then
		runePowerStatusBarText = _G["RunePowerStatusBarText"]
	end
	if not RuneStatusBar then
		RuneStatusBar = _G["RuneStatusBar"]
	end

	-- Still nil? Abort.
	if not runePowerStatusBar then return end
